...
import 'package:blog_application_aastu_grp3/features/article/domain/usecases/edit_article_usecase.dart';
...
class AddArticleBloc extends Bloc<AddArticleEvent, AddArticleState> {
  ...
  AddArticleBloc() : super(AddArticleInitial()) {
    ...
    EditArticleUsecase editArticleUsecase = EditArticleUsecase(articleRepository: repository);
    ...
    on<EditArticlesEvent>((event, emit) async {
      print('ADFADKF');
      emit(ArticleLoading());

      try {
        final result =
            await editArticleUsecase(EditArticleParams(article: event.article));
        result!.fold((faliure) => emit(AddArticleFailed()),
            (article) => emit(ArticleLoaded(article: article)));
      } catch (e) {
        emit(AddArticleFailed());
